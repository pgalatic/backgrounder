...
class Config_Subs():
	...
	def __init__(self, frame):
		...
		customsub_lbl = tk.Label(customsub_frame, text='BETA: (Optional) Enter a custom subreddit.')
		customsub_lbl.grid(row=0, column=0, sticky=tk.W)
		
		customsub_input = tk.StringVar(customsub_frame)
		
		customsub_txt = tk.Entry(customsub_frame, textvariable=customsub_input, width=30)
		customsub_txt.grid(row=1, column=0, sticky=tk.W)
		
		customsub_btn = tk.Button(customsub_frame, text='Add', width=10, command=lambda: self.add_custom_subreddit())
	...
	def add_custom_subreddit(self):
		...
		var = tk.IntVar()
		var.set(1)
		customsub_box = tk.Checkbutton(self.customsub_frame, text=subname, variable=var)
